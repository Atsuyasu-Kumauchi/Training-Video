...
export default function AdminLoginView(props: TAdminLoginViewSchema) {
  ...
  const {
    control,
    formState: { errors },
  } = useFormContext<TAdminLoginSchema>();
  return (
    <>
      {/* Logo and Header */}
      <div className="text-center">
        <h2 className="mt-6 text-3xl font-bold text-gray-900">
          {lang.auth.login.admin_login}
        </h2>
        <p className="mt-2 text-sm text-gray-600"> {lang.auth.login.tms}</p>
      </div>
      {/* Login Form */}
      <div className="mt-8 space-y-6">
            ...
            render={({ field }) => (
              <div>
                ...
                <input
                  ...
                  placeholder={lang.auth.login.emailLabel}
                  {...field}
                />
              </div>
            )}
            ...
            render={({ field }) => (
              <div>
                ...
                <input
                  ...
                  placeholder={lang.auth.login.passwordLabel}
                  {...field}
                />
              </div>
            )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            >
              {lang.auth.login.stay_logged_in}
            </label>
          </div>
        ...
        <div>
          <Button
            ...
            loading={isPending}
          >
            <span> {lang.auth.login.login}</span>
          </Button>
        </div>
      </div>
      {/* Error Message */}
      {errorMessage && (
        <div
          id="error-message"
          className="bg-red-50 border border-red-200 rounded-md mt-4 p-4"
        >
          <div className="flex">
      ...
      <div className="text-center mt-4">
        <p className="text-xs text-gray-500">
          © {new Date().getFullYear()} Training Management System. All rights
          reserved.
        </p>
      </div>
    </>
