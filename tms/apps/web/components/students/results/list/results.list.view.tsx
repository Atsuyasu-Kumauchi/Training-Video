...
import ResultsListDetails from "./results.list.details";
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
      <div className="px-6 py-4 border-b border-gray-200">
        <h2 className="text-lg font-medium text-gray-900">
          {" "}
          {result.list.list_title}
        </h2>
        <p className="text-sm text-gray-500 mt-1">
          {" "}
          {result.list.list_subTitle}
        </p>
      </div>
      <div className="p-6">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center space-x-6 mt-2 text-sm text-gray-500">
                    <span>
                      ...
                      {result.list.completed}: {results.completedDate}
                    </span>
                  </div>
                  <div className="mt-3">
                    <Button
                      onClick={() => openResultDetails(results.id)}
                      variant="outline"
                      color="neutral"
                    >
                      <FontAwesomeIcon icon={faEye} className="mr-2" />
                      {result.list.view_detail}
                    </Button>
                  </div>
                </div>
              </div>
      ...
      <UiBasicModal
        ...
        description=""
        // body={<div>{result.list.question_details}</div>}
        body={
          <div>
            <ResultsListDetails />
          </div>
